...
void Form::drawML () {
	...
	if (ImGui::Button("Next##asdf", ImVec2{window.x * .2f, (font->FontSize * 2) - (ImGui::GetStyle().WindowPadding.y * 2.0f)})) {
		...
		// copy over the hansel chain set, but it's just a vector of ints now...
		...
		func->hanselChains->attributes.resize(edm->attribute_names.size()); // copy the attributes over
		...
		for (const auto hc : edm->hanselChainSet)
		{
			...
			for (auto e : hc) // copy by value
			{
				...
				t[t.size() - 1].push_back(e.dataPoint);
			}
		}
		func->hanselChains->hanselChainSet = t;
	}	
